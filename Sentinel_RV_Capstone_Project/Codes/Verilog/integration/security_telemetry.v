...
// Buffers ADC/status telemetry for System Logger and emits AES-128 keyed event-chain
// digests for the SD audit log. The AES operation is a one-block PRF:
...
module security_telemetry (
    ...
    input  wire         transport_error,
    input  wire [7:0]   result_opcode,
    input  wire [7:0]   result_sequence,
    input  wire [15:0]  result_argument,
    input  wire [7:0]   result_source_id,
    input  wire         alarm,
    ...
    output reg [127:0]  audit_metadata
);
        ...
        if (reset) begin
            ...
            if (result_valid) begin
                ...
                if (!audit_busy && !audit_request) begin
                    audit_metadata <= {8'h01, result_source_id, result_opcode, result_argument,
                                       result_sequence, {5'd0, transport_error, alarm, result_accepted},
                                       {4'd0, adc_sample}, dht11_temp, dht11_hum, distance, 24'd0};
                    audit_block <= previous_digest ^
                                   {8'h01, result_source_id, result_opcode, result_argument,
                                    result_sequence, {5'd0, transport_error, alarm, result_accepted},
                                    {4'd0, adc_sample}, dht11_temp, dht11_hum, distance, 24'd0};
                    audit_start <= 1'b1;
                    audit_busy <= 1'b1;
