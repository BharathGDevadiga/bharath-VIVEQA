...
module sentinel_rv_board_top #(
    ...
    input wire sw_alarm_test,      // S4 (A7) - Manual Alarm Buzzer Test
    ...
    // Periodic 1ms tick drives the XADC monitor so tamper detection runs continuously
    localparam integer XADC_TICK_CYCLES = 24_000 - 1;  // 1 ms at 24 MHz
    reg [14:0] xadc_tick_counter;
    reg xadc_sample_tick;
    wire [3:0] unused_keypad_row;  // keypad rows not needed on board_top

    // Stub XADC codes — within safe window; replace with XADC Wizard outputs if available
    localparam [11:0] XADC_VCCINT_STUB  = 12'h550;  // ~0.99 V safe
    localparam [11:0] XADC_TEMP_STUB    = 12'h500;  // ~65 °C safe
    ...
    // XADC sampling tick: 1 ms periodic pulse to keep tamper detection alive
    always @(posedge clk_24mhz) begin
        if (reset) begin
    ...
    // L1 = System Run Status (!reset)
    // L2 = Alarm Clear Status
    // L3 = Display Lock Status
    // L4 = Alarm Test Status / Alarm
    // L5 = Relay Status (relay_in)
    // L6 = CPU Heartbeat
    // L7 = SD Card Logging Active
    // L8 = Master Clock Heartbeat (24 MHz)
